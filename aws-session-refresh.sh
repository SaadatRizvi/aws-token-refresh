...
aws sts get-session-token --serial-number "arn:aws:iam::{aws_account_ID}:mfa/{IAM_user_name}" --profile {profile} --output text --token-code "$1"|awk  '{print "\naws_access_key_id=\""$2"\"" "\naws_secret_access_key=\""$4"\""  "\naws_session_token=\""$5"\""  }'  2>&1 | tee  ~/.aws/creds.current.vals
...
for i in {1..2}
do
	sed -i '0,/aws_access_key_id1/s/.*aws_access_key_id1.*/aws_access_key_id = {aws_access_key_id} \n/' ~/.aws/credentials
	sed -i '0,/aws_secret_access_key1/s/.*aws_secret_access_key1.*/aws_secret_access_key = {aws_secret_access_key} \n/' ~/.aws/credentials
	sed -i '0,/aws_session_token1/s/.*aws_session_token1.*/aws_session_token = {aws_session_token} \n/' ~/.aws/credentials
done
